...
export function fetchRequest(slug) {
    return apiGet(`/r/${slug}`);
}
 ...
 */
export function uploadFile(slug, file, submitterName, message, onProgress) {
    return new Promise((resolve, reject) => {
        const fd = new FormData();
        fd.append('file', file);
        if (submitterName.trim())
            fd.append('submitter_name', submitterName.trim());
        if (message.trim())
