...
CREATE TABLE transfer.transfer_files (
    transfer_id UUID NOT NULL REFERENCES transfer.transfers(id) ON DELETE CASCADE,
    file_id     UUID NOT NULL,
    PRIMARY KEY (transfer_id, file_id)
);

CREATE INDEX idx_transfers_owner    ON transfer.transfers(owner_id);
CREATE INDEX idx_transfers_slug     ON transfer.transfers(slug);
CREATE INDEX idx_transfers_expires  ON transfer.transfers(expires_at) WHERE expires_at IS NOT NULL;
